...
%% almost as usual
\author{Gavin L. Simpson\\Department of Animal and Veterinary Science\\Aarhus University}
...
\usepackage{textcomp}
\newcommand{\glstextapprox}{\raisebox{0.5ex}{\texttildelow}}
...
This example, of multiple species responses along a single environmental gradient, illustrates the simplest usage of \texttt{coenocline()}. The example uses a hypothetical pH gradient with species optima drawn at random uniformally along the gradient. Species tolerances are the same for all species. The maximum abundance of each species, $h$, is drawn from a lognormal distribution with a mean of \glstextapprox{}20 ($e^3$). This simulation will be for a community of 20 species, evaluated at 100 equally spaced locations. First we set up the parameters
